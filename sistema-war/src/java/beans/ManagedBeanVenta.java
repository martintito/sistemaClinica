...
public class ManagedBeanVenta implements Serializable {
    ...
    private Integer codigo_venta = 0;
    ...
    public String ReportesCobros() {
        return "reporte_cobros_ventas";
    }
...
// te quedaste aqui
    ...
    public List<Producto> completar_filtrado_Producto(String nombre) {
        lista_Productos = new LinkedList<Producto>();
        lista_Productos = ProductoFacade.filtrado_simple("nombreProducto", nombre);
    ...
    public List<Cliente> completar_filtrado_Clientes(String nombre) {
       ...
       lista_clientes= clienteFacade.filtrado_simple("nombreCliente", nombre);
...
// OODIGO AGREGADO PARA LECTOR DE CODIGO DE BARRAS.
    ...
    public void buscar_venta(Integer cod_venta_) {
        venta = new Venta();
        try {
            venta = ventaFacade.find(cod_venta_);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List<DetalleVentaProducto> traer_detalles_venta(Venta v) {
        List<DetalleVentaProducto> resp_ = new LinkedList<DetalleVentaProducto>();
        try {
            resp_ = detalleVentaProductoFacade.buscar_objeto("venta", v, "idDetalleVentaProducto", 0);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return resp_;
    ...
    public void elimina_detalle(DetalleVentaProducto det_) {
        // hay que validar para que agrege al stock de almacen
        detalleVentaProductoFacade.remove(det_);
    }
    ...
    public void editar(Venta v) {
        try {
            ...
            ventaFacade.edit(v);
            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "ACTUALIZADO CON EXITO", "DATOS CORRECTOS");
            FacesContext.getCurrentInstance().addMessage(null, message);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void selecciona_detalle_sacar(DetalleVentaProducto obj_) {
        detalle_venta_Producto = obj_;
    }

    public void eliminar_detalle_venta_(DetalleVentaProducto det_venta) {
        try {
            // vamos a aregar el stock
            StockProductoTiendaOrigen objetoStockProductoTiendaOrigen = new StockProductoTiendaOrigen();
            objetoStockProductoTiendaOrigen = (StockProductoTiendaOrigen) stockProductoTiendaOrigenFacade.buscar_object_dosParametros("tienda", det_venta.getVenta().getTienda(), "producto", det_venta.getProducto());
            objetoStockProductoTiendaOrigen.setCantidad(objetoStockProductoTiendaOrigen.getCantidad() + det_venta.getCantidad());
            stockProductoTiendaOrigenFacade.edit(objetoStockProductoTiendaOrigen);

            IngresoProductoTienda ingresotiendaProducto = new IngresoProductoTienda();
            ingresotiendaProducto.setCantidad(det_venta.getCantidad());
            ingresotiendaProducto.setCostoUnitario(det_venta.getPrecioVenta());
            ingresotiendaProducto.setFechaIngreso(new Date());
            ingresotiendaProducto.setFechaVencimiento(new Date());
            ingresotiendaProducto.setUbicacionFisica(new UbicacionFisica(1));

            ingresotiendaProducto.setProducto(objetoStockProductoTiendaOrigen.getProducto());
            ingresotiendaProducto.setTienda(objetoStockProductoTiendaOrigen.getTienda());
            ingresotiendaProducto.setMotivo("ELIMINACION EN VENTA:" + det_venta.getVenta().getIdVenta());
            ingresoProductoTiendaFacade.create(ingresotiendaProducto);

            // ACTUALIZAMOS LA VENTA
            venta.setTotalVenta(venta.getTotalVenta().subtract(det_venta.getSubTotal()));
            ventaFacade.edit(venta);
            // PROCEDEMOS A ELIMINAR
            detalleVentaProductoFacade.remove(det_venta);
            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "ELIMINADO CON EXITO", "PROCESO CORRECTO");
            FacesContext.getCurrentInstance().addMessage(null, message);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void preparaNewDetalle() {
        detalle_venta_Producto = new DetalleVentaProducto();
    }

    public void agrega_detalle_directo() {
        try {
            detalle_venta_Producto.setDescuento(BigDecimal.ZERO);
            detalle_venta_Producto.setVenta(venta);
            BigDecimal cantidad = new BigDecimal(detalle_venta_Producto.getCantidad());
            detalle_venta_Producto.setSubTotal((detalle_venta_Producto.getPrecioVenta().subtract(detalle_venta_Producto.getDescuento())).multiply(cantidad));
            detalleVentaProductoFacade.create(detalle_venta_Producto);
            // ACTUALIZAMOS LA VENTA
            venta.setTotalVenta(venta.getTotalVenta().add(detalle_venta_Producto.getSubTotal()));
            ventaFacade.edit(venta);
